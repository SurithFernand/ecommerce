...
//    }
    ...
    @GetMapping("/list") // This maps to the URL http://localhost:8080/products/list
    public String listProducts(Model productListModel) { // Model argument is used to pass data to the view
        productListModel.addAttribute("products", productService.getAllProducts()); // Add the productsList to the model
...
//    }
    ...
    @GetMapping("/add")    // Maps to the URL http://localhost:8080/add
    public String showProductForm(Model productAddFormModel){
        Product product = new Product();
        productAddFormModel.addAttribute("product", product);     //Should make a constructor without parameters at the model
